...
⚠ **הם אינם ב־repo, ובכוונה.** הם יושבים ב־`.env.deploy` המקומי, שמוחרג
מ־git. `scripts/write-deploy-config.mjs` קורא אותו בזמן הבנייה וצורב את
הערכים לקובץ שרת בלבד.

```
SEED_ADMIN_PASSWORD=...    סיסמת הצוות בפריסה
APP_API_KEY=...            שער פתיחת סשן מהאפליקציה
```

⚠ **הקובץ אינו מגובה בשום מקום.** אם המחשב אבד, יש ליצור ערכים חדשים
ולפרוס מחדש — הסיסמה מסונכרנת אוטומטית לכל חשבונות הצוות בעלייה הבאה.

משתנה סביבה גובר על הקובץ, כדי שפריסה ממערכת CI תוכל להזין ערכים משלה
בלי לשנות קוד.

## מאגר הקוד

```
https://github.com/orishushan207/velax-ops-console   (פרטי)
```

⚠ היסטוריית ה־git נוקתה מסודות לפני הדחיפה הראשונה. `netlify.toml`
ו־`scripts/setup-remote-db.sh` הכילו סיסמה בגרסאות קודמות; שתיהן הוחלפו
ב־`<REDACTED>` לאורך כל ההיסטוריה.
